...
const CurrentExhibitionInfo: React.FC<CurrentExhibitionInfoProps> = React.memo(({ uiConfig, isLoading, activeExhibition, isInfoOpen, isSmallScreen, isCurrentExhibitionInfoHidden, onInfoOpen }) => {
    ...
    return (
        <React.Fragment>
        <div className={`absolute left-10 z-40 transition-all duration-500 max-w-xl ${isInfoOpen ? '-translate-x-full opacity-0' : slideOutClass} bottom-28 md:bottom-10`}>
            {isLoading ? (
                <div className={`flex items-center gap-3 ${uiConfig.text} animate-pulse`}>
                    <Loader2 className="w-6 h-6" />
                    <span className="text-xl font-serif">Loading Exhibition...</span>
                </div>
            ) : (
                <React.Fragment>
                    <div className={`flex items-center gap-2 mb-4 ${uiConfig.subtext}`}>
                    ...
                    <h2 className={`text-4xl md:text-5xl font-serif font-medium leading-tight uppercase ${uiConfig.text} mb-2`}>
                    ...
                    <div className="flex items-center gap-4 mb-2">
                        ...
                        <p className={`text-sm md:text-base font-light tracking-widest ${uiConfig.subtext} border-l-2 ${uiConfig.lightsOn ? 'border-neutral-300' : 'border-neutral-700'} pl-4`}>
                            {activeExhibition.subtitle}
                        </p>
                    </div>
                    <div className={`text-[12px] font-mono opacity-60 mt-2 ${uiConfig.text}`}>
